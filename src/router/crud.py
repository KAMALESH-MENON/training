...
@router.get("/{pokemon_id}", status_code=status.HTTP_200_OK, response_model=PokemonOutput)
def read_pokemon(pokemon_id: int):
    _service = service()
    return _service.read_pokemon_service(pokemon_id=pokemon_id)

@router.put("/{pokemon_id}", status_code=status.HTTP_200_OK, response_model=PokemonOutput)
def update_pokemon(pokemon_id: int, pokemon: PokemonUpdate):
    _service = service()
    return _service.update_pokemon_service(pokemon_id, pokemon)

# @router.patch("/{pokemon_id}", response_model=PokemonOutput)
# def partial_update_pokemon(pokemon_id: int, pokemon: PokemonUpdate):
#     return service.update_pokemon_service(pokemon_id, pokemon)
